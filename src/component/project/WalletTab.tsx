        ...
        <div className="bg-dark-surface p-6 rounded-xl border border-border-color animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 text-center">
                <div className="bg-dark-bg p-4 rounded-lg">
                    ...
                    <p className="text-2xl font-bold text-white mt-1">{new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(project.totalFee || 0)}</p>
                </div>
                <div className="bg-dark-bg p-4 rounded-lg">
                    <p className="text-sm text-text-secondary">Đã thanh toán</p>
                    <p className="text-2xl font-bold text-green-400 mt-1">{new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(project.balance || 0)}</p>
                </div>
                <div className="bg-dark-bg p-4 rounded-lg">
                    <p className="text-sm text-text-secondary">Còn lại</p>
                    <p className="text-2xl font-bold text-yellow-400 mt-1">{new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format((project.totalFee || 0) - (project.balance || 0))}</p>
                </div>
            </div>
